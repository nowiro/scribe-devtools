...
Pierwsze narzędzie: **browser-inspector 2** (`bi`) — patrzenie na aplikację webową przez
prawdziwy, systemowy Chrome/Edge, z dwoma wejściami do jednej tabeli kroków:

| wejście | do czego | co dostaje agent |
| --- | --- | --- |
| `bi <config.json> [--stamp X]` | **batch**: flow z configu JSON, drop-in dla bramki app-factory; wynik w `<outputDir>/<stamp>/<snapshot>/report.md` + `report.json` | jedna linia stdout na snapshot + `report.md` do przeczytania |
| `bi <komenda> [args]` | **sesja interaktywna**: `bi open <url>`, `bi find <tekst>`, `bi click e45`, `bi snap`, `bi export flow.json` | jedna linia stdout na komendę |
...
potem kliknąć”. `bi` robi jedno i drugie: batch jak stary runner (ten sam config, ten sam
...
Wywołanie bez `bi` na PATH: `node packages/browser-inspector/bin/bi.mjs …` albo `npm run bi -- …`.
W innym projekcie (np. app-factory) dodaj skrypt
`"bi": "node ../scribe-devtools/packages/browser-inspector/bin/bi.mjs"` i wołaj **`pnpm bi …`**.
...
```
bi read.config.browser-inspector.json --stamp 2026-09-02_10-00
...
`bi lint-config <config.json>` drukuje sugestie migracji (na configu app-factory: `networkidle` →
...
## Sesja: `bi open`, `bi find`, `bi click eN`, `bi snap`
...
**tylko nazwa** (`--session`, `BI_SESSION`, domyślnie `default`) — `cd` w powłoce nie otwiera drugiej
...
`video/`). Dialogi: polityka PRZED akcją (`bi dialog accept|dismiss [--text] [--once]`),
`beforeunload` zawsze akceptowany. `bi frame <n>` zmienia zakres tylko dla selektorów CSS,
`eval` i `extract` — refy z ramek rozwiązują się bez niego. `bi run --file s.mjs` (kod w keeperze)
istnieje wyłącznie pod `BI_UNSAFE=1` i jest jawnie RCE-równoważny — nigdy w configu ani w CI.

`bi export flow.json` zapisuje dziennik sesji jako config batchu: refy → trwałe selektory
...
nie w bezimienne `generic`). Dla CI bez keepera: `bi script plik.txt --no-daemon` wykonuje te same
linie w jednym procesie (stop na pierwszym FAIL, exit = pierwszy niezerowy).

## Keeper: `bi up`, `bi status`, `bi stop`, `bi doctor`

Keeper startuje sam przy pierwszym `bi` (także batchu). W sesji agenta warto go podnieść jawnie
jako hook **SessionStart**, żeby pierwsze wywołanie w sesji było już ciepłe — np. w
`.claude/settings.json`:

```json
{ "hooks": { "SessionStart": [{ "hooks": [{ "type": "command", "command": "pnpm bi up" }] }] } }
```

`bi status` drukuje pid, hash tożsamości, pipe, RSS, liczbę zadań, lane’y i sesje; `bi stop`
zamyka; `bi doctor` sprawdza, czy keeper **przeżywa wyjście powłoki** (część hostów — VS Code,
...
flag, proxy, `realpath(bin/bi.mjs)` i stempla mtime `src/**` — edycja źródeł albo drugi checkout
dostają **inny** pipe, stary keeper nie jest już trafiany. Plik pid (`bi-<hash>.json` z tokenem),
...
Zmienne środowiskowe: `BI_DAEMON=0` (bez keepera) / `BI_DAEMON=1` (keeper mimo CI), `BI_SOCKET`
(własny pipe — dwa agenty, dwa keepery), `BI_SESSION`, `BI_IDLE_MS` (30 min), `BI_SESSION_TTL_MS`
(60 min), `BI_LANE_IDLE_MS`, `BI_MAX_JOBS`, `BI_MAX_RSS_MB` (recykling przeglądarki **między
zadaniami, przy pustej kolejce, bez otwartej sesji i zajętego lane’u** — należny recykling czeka na
`bi close`; RSS Chrome + rendererów próbkowany w tle co 10 zadań, nigdy na ścieżce zadania),
`BI_CHANNEL`, `BI_BROWSER_PATH`, `BI_BROWSER_ARGS`, `BI_STEP_TIMEOUT_MS` (limit kroku sesji),
`BI_SCRUB_OP_MS` (limit jednej operacji scrubu, 2000 — zawieszony renderer kończy się odbudową
karty, nie zawieszonym lane’em), `BI_REQUEST_TIMEOUT_MS` (klient porzuca keepera bez żadnej linii
przez 10 min: batch idzie w procesie, sesja `exit 2`), `BI_UNSAFE` (**wchodzi do hasha
tożsamości** — keeper z `BI_UNSAFE=1` to osobny proces, którego zwykły klient nie trafi);
testowe: `BI_TMPDIR` (pliki pid/lock/log), `BI_CONNECT_TIMEOUT_MS`, `BI_ENGINE_MODULE`,
`BI_SKIP_SMOKE`, `BI_PERF`, `APP_FACTORY_DIR`.

Pliki pid/lock w `%TEMP%` przeżywają restart, a Windows recyklinguje pidy: keeper uznaje plik z
„żywym” pidem za stary, gdy ma ponad 5 s i nikt nie odpowiada na pipe (sam go sprząta); `bi stop` /
`bi status` bez keepera wypisują ścieżkę takiego pliku.
...
`bi script`. `--junit plik.xml` + `--fail-on-incomplete` robią z batchu bramę:

```
node ../scribe-devtools/packages/browser-inspector/bin/bi.mjs read.config.json --stamp "$STAMP" --junit bi.xml --fail-on-incomplete
```

Bramka app-factory (`pnpm smoke:browser`) woła `bi` dokładnie tak (stary argument `--stamp X`
...
1. Config bez zmian; zamiast `node <scribe>/integrations/dist/browser-inspector/read-browser-inspector.js <config> --stamp X`
   wołaj `node <scribe-devtools>/packages/browser-inspector/bin/bi.mjs <config> --stamp X` (albo
   `pnpm bi <config> --stamp X`). Katalog wyniku, `report.json` (`completed`, `steps[].description/ok/error`,
   `navigationError`), `_manifest.json` — bez zmian; `report.md` jest krótszy (sekcje warunkowe).
2. `bi lint-config` mówi, co warto zmienić w configu (`settled`, `waitFor`, `parallel`). Nowe kroki:
...
5. Nowość, której scribe nie miał: sesja interaktywna i `bi export` — patrz wyżej.

## Układ repozytorium

```
packages/browser-inspector/   # @scribe-devtools/browser-inspector — bin/bi.mjs, src/, test/, fixtures/, templates/
bench/                        # pomiar bi vs @playwright/mcp 0.0.80: RAPORT.md, WYNIKI.md, BUDGET.md, probes/
...
| `npm run bi -- <args>` | `bi` bez PATH |

<!-- BENCH:START -->

Pomiar z 2026-09-02T02:10:21.641Z (`npm run bench`, Chrome/152, @playwright/mcp 0.0.80):

| wariant | mediana | p90 | × vs MCP naive | × vs MCP lean settle 100 |
| --- | ---: | ---: | ---: | ---: |
| bi-warm (2.+ wywołanie, przerwa 300 ms) | **358 ms** | 366 ms | 10,8× | 4,6× |
| bi-warm-tight (bez przerwy) | **355 ms** | 370 ms | 10,9× | 4,6× |
| bi-first (keeper startuje w stoperze) | **1385 ms** | 1395 ms | 3,5× (vs 1. przebieg) | 1,4× (vs 1. przebieg) |
| bi-cold (`--no-daemon`, CI) | **2077 ms** | 2145 ms | 2,3× (vs 1. przebieg) | 0,9× (vs 1. przebieg) |
| MCP naive / lean / lean settle 100 (warm) | 3871 / 3898 / 1648 ms | — | — | — |

Tokeny (o200k) na sesję z jednym zadaniem: **bi batch 400** (blok AGENTS.md 146 + komenda, stdout i cały `report.md`), bi-interactive-naive 745, bi-interactive-lean 558 — wobec MCP naive 6814 / lean 5499. Szczegóły: [bench/RAPORT.md](bench/RAPORT.md), budżet vs pomiar: [bench/BUDGET.md](bench/BUDGET.md).
