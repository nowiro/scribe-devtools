...
| AGENT-12  | low    | CWE-532      | `src/engine.mjs:957`                              | CONFIRMED, repro                           | `snap.md`/`snap.full.yml`/`snap.json` omijają pełny `ctx.redact` — sekret echem strony poza polem wartości zostaje w pliku.                             |
...
#### AGENT-12 — Pliki `snap.*` omijają pełny `ctx.redact`, choć stdout tego samego snapshotu jest redagowany (low)
...
- **Pola hasła i OTP bez wartości w snapshotach.** `sensitive = type==='password' || autocomplete==='one-time-code'`
  (`snapshot.mjs:836`) → `maskSnapshotValues`/`maskSnapshotEntries` (`redact.mjs:90-123`) usuwają `= wartość` z
  `snap.md`, `snap.json`, `snap.full.yml` — potwierdzone w teście na żywo.
