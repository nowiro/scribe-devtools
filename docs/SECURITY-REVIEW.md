...
| SECRETS-2 | medium | CWE-116      | `src/redact.mjs:17`                               | NAPRAWIONE cz. (formy w `secretForms`)     | `secretForms` nie zna formy `x-www-form-urlencoded` ani base64 — hasło ze spacją lub `!` przechodziło przez `redact()`; formy dodane, nagłówek `Basic` (`base64(user:hasło)`) zostaje w SECRETS-1.                                    |
...
#### AGENT-14 — Cztery resztkowe drogi wartości pola hasła do `snap.md` / `snap.full.yml` (medium) — NAPRAWIONE

**Status.** Naprawione; AGENT-12 zamknął dwie z sześciu ścieżek tego samego niezmiennika §2.6, te cztery zostały.

1. **Ref sprzed nazwy dostępnej** (`src/redact.mjs:133`). `yamlValueLine` brało PIERWSZE `[ref=…]` w kluczu, a nazwa
   jest tekstem strony i stoi przed prawdziwym refem: `aria-label="Hasło [ref=e1] konta"` podstawiało obcy ref, więc
   `sensitiveRefs` chybiało i hasło zostawało w `snap.full.yml`. W drugą stronę — nazwa wspominająca ref wrażliwy —
   ucinało wartość polu, które wrażliwe nie jest (cicha utrata danych). Ref czyta się teraz dopiero za nazwą (`afterName`),
   dla klucza gołego i cytowanego apostrofami tak samo.
2. **Wartość w slocie nazwy i w tekście kontenera** (`src/snapshot.mjs`). Maska zna w kompakcie dokładnie jeden slot
   (` = wartość`); `renderLine` awansowało wpisaną wartość do slotu NAZWY dla pola bez etykiety, a `textUnder`
   wciągało ją w komunikat `dialog`/`alert`. `textUnder` pomija teraz tekst ról niosących wartość.
3. **Parowanie ramek po pozycji** (`src/snapshot.mjs`, `sidecarFromPage`). i-ty węzeł `iframe` szedł do i-tego wpisu
   `page.frames()`; ramka `display:none` (analityka, zgody) jest w drugiej liście i nie ma jej w pierwszej, więc widżet
   logowania dostawał obchód cudzego dokumentu i pole traciło `sensitive`. Ramkę rozwiązuje się teraz po WŁASNYM refie
   węzła (`aria-ref=eN` → `contentFrame()`).
4. **Nieudany obchód DOM = „nic nie jest wrażliwe”** (`src/steps.ctx.mjs`). `degradeTo` łykało wyjątek i sidecar bez ani
   jednej flagi `sensitive` przechodził dalej jako prawda. Teraz nieznana wrażliwość tnie wartość każdej linii o roli
   niosącej wartość (fail-closed), i ten sam znacznik jedzie w `ctx.lastSnapshot`, żeby stdout sesji i `snap --diff` szły
   za plikiem.

**Regresja.** `redact.test.mjs` (ref w nazwie, oba kierunki, klucz cytowany, `maskAllValueRoles`),
`snapshot.test.mjs` (pole bez etykiety, `dialog`, parowanie ramek wobec `display:none` i odwróconej kolejności
podpięcia), `engine.test.mjs` (obchód, który rzuca), smoke na prawdziwym Chrome: `fixtures/iframe-blank.html`
