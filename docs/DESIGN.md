...
- **Rejestrator** (`src/recorder.mjs`): podpięty RAZ na stronę przed nawigacją (`console`, `pageerror`, `request/requestfinished/requestfailed`, `response`, `dialog`, `popup`, `framenavigated`, `crash`); kursory „od ostatniego wywołania” per sesja; przechowuje ciała odpowiedzi ≤ 64 KB dla `application/json|text/*` dla `browser-inspector net <n> --body` — **w sesji**; w batchu `captureBodies` jest opt-in (`false` domyślnie), bo żaden artefakt raportu ciała nie renderuje, a czekanie na nie kosztowało 41–57 ms z 232 ms przebiegu. `size` wpisu nie zależy od tego odczytu: bierze się z `Content-Length`, a przy `chunked` z `request.sizes()` (`encodedDataLength`, bez dodatkowego obiegu) — są to bajty **na łączu**, nie zdekodowane. Żądanie, które w chwili budowania raportu **wciąż jest w locie**, nie ma ani `ms`, ani `size` (oba wypełnia `requestfinished`, a batch na niego nie czeka) — status i URL są, bo przyszły z odpowiedzi; nigdy nie loguje wartości sekretów (§2.6).
- Każdy krok jedzie przez `withDeadline` (port z wersji TS). `evaluate`/`eval` = CDP `Runtime.evaluate({ expression, returnByValue: true, awaitPromise: true, timeout })` **opięte także `withDeadline`**, bo `timeout` CDP przerywa tylko część synchroniczną — wisząca obietnica kończy się deadlinem kroku (strona żyje). Wynik obiektowy dojeżdża **drugim** wywołaniem, `Runtime.callFunctionOn({ functionDeclaration: 'function () { return JSON.stringify(this); }', returnByValue: true })` — serializacja dzieje się **w stronie**, bo `returnByValue` CDP wylicza tylko własne wyliczalne właściwości i nie woła `toJSON`, więc `new Date(0)` wracało jako `{}`, a `{ stan: 1, od: new Date() }` jako `{"stan":1,"od":{}}` (jedno pole cicho puste, krok `ok`). Mapowanie wyniku 1:1 ze starym `page.evaluate(expression)`: `undefined` → literał `undefined`, string → bez zmian, reszta → `JSON.stringify` (`Date` → ISO, funkcja i symbol → `undefined`, struktura cykliczna → FAIL kroku z `TypeError` ze strony). Zmierzone odstępstwa, świadome: `NaN`/`Infinity`/`BigInt` wracają zapisem CDP (`"NaN"`, `"7n"`) zamiast `null`/wyjątku, a `new Error("e")` daje `{}` zamiast `{"name":"Error"}` — Node rekonstruował Error po swojej stronie, `JSON.stringify` w stronie tego nie odtworzy. Wartość nieserializowalna (DOM node) → FAIL kroku z komunikatem CDP; „Internal error" z **drugiego** wywołania to `evaluate result is not serializable (<klasa>)`, nigdy „timed out" — tam nie ma parametru `timeout`, a prawdziwy zwis łapie `withDeadline`; wyjątek → FAIL z `exceptionDetails.exception.description` obciętym do pierwszej linii (`Error: uczen widzi przycisk nauczyciela` — dokładnie forma, którą drukuje dziś bramka; fixture w `test/compat`).
- Zrzuty: CDP `Page.captureScreenshot({ format, quality, optimizeForSpeed: true })` — **także `fullPage`**, przez `captureBeyondViewport` z klipem z `Page.getLayoutMetrics.cssContentSize`; element i nie-Chromium → `page.screenshot`, tak samo strona wyższa niż 16 384 px (limit tekstury Chrome) i strona, której metryki wyglądają na zerowe. Zmierzone A/B na najwyższej stronie fixture'ów (bookstore, 1280×6335): CDP 222 ms, Playwright 713 ms, wymiary identyczne — ale **cała ta różnica to enkoder PNG, nie obiegi**: ten sam klip CDP z `optimizeForSpeed: false` kosztuje 731 ms. Kupujemy czas rozmiarem pliku (3488 KB wobec 2045 KB dla tego samego obrazu; na `nowiro-strona` 510 → ~850 KB przy `captureMs` 614 → ~350 ms) i jest to ten sam wybór, który zrzuty viewportu robią od początku. PNG domyślnie, JPEG opt-in (w CDP nie jest szybszy). Zapis asynchroniczny równolegle z krokami; przed `report.json` keeper czeka na wszystkie zapisy.
...
5. kontekst: `clearCookies`, `clearPermissions`, `unrouteAll`, `setOffline(false)`, `setExtraHTTPHeaders({})`, `setGeolocation(null)`, `page.emulateMedia({ colorScheme: null, reducedMotion: <`reduce`, gdy `browser.motion` tego żąda, inaczej `null`>, media: null })` (samo `null` idzie do przeglądarki jako `no-override` i kasuje też opcję KONTEKSTU, więc scrub wyłączał `browser.motion: "reduce"` do końca życia karty), viewport domyślny, polityka dialogów `dismiss`, `setDefaultTimeout/NavigationTimeout` domyślne — razem z 4. dla 4 originów **34 ms**, dla jednego ≈ 12 ms;
...
Co zostaje celowo: cache HTTP, code-cache V8, ciepły renderer. Statyczny `serveStatic` app-factory nie wysyła `ETag`/`Last-Modified`/`Cache-Control` (sprawdzone), więc Chrome nic z niego nie cache’uje; **nginx/`serve` wysyłają `Last-Modified` i heurystyczna świeżość (10 % wieku) może podać stary `index.html` po rebuildzie** — dlatego silnik liczy `cacheHits` (`timing.cacheHits`, osobno `cacheHitsDocument`) — z **Resource Timing API strony**, w tym samym przelocie co dowód końcowy (`capture.mjs`), a nie z rejestratora: `response.fromCache()` **nie istnieje** w playwright-core 1.62.1, więc licznik oparty na nim raportował 0 w każdym zmierzonym przebiegu, a `safeCall` połykał wyjątek. Wpis jest trafieniem, gdy `deliveryType === 'cache'` albo `transferSize === 0 && decodedBodySize > 0`; liczone per dokument, bo bufor Resource Timing zeruje się przy nawigacji. Dokument dodatkowo musi mieć `responseStatus > 0`: wewnętrzna strona błędu Chrome (`chrome-error://chromewebdata/` po `net::ERR_*`) raportuje `deliveryType: 'cache'` i `transferSize: 0` dla ciała, które sama zmyśliła — nieudana nawigacja meldowała wtedy „dokument z cache”, czyli dokładnie ostrzeżenie o starym buildzie, którego nie było. BUDGET.md czerwieni wiersz > 0 dla dokumentu głównego, a `isolation: "fresh"` / `--fresh` daje świeży kontekst z puli spare + `Network.clearBrowserCache`. `report.json.timing` mówi `ctx: reused | fresh`, `tab: kept | new`.

`needsFreshContext(snapshot)` = `isolation === 'fresh'` ∨ `auth`/`storageState` ∨ `video` ∨ `--fresh`. `--parallel N`: lane’y `scratch[1..N-1]` są **takimi samymi trwałymi kontekstami** (nie świeżymi — świeży kontekst kosztuje 190–441 ms na pierwszym goto), tworzone przy pierwszym użyciu, szorowane tak samo, zamykane po `BROWSER_INSPECTOR_LANE_IDLE_MS` (300 000). Dla app-factory z `parallel: 3` czas ≈ max(lane), nie sum(flow) — i żeby to była prawda, a nie obietnica, przydział robi `planLanes` (`src/schedule.mjs`): offline LPT (najdroższe najpierw, do najmniej obciążonego lane'u) po szacunku liczonym **czystą funkcją z configu** (`Σ wait.ms + 100 × liczba kroków`), nigdy z historii na dysku. Round-robin `k % parallel` sadzał snapshoty 0 i 3 na jednym lane'ie niezależnie od kosztu: 4,35 s wobec 3,24 s na tym configu. Zmienia się **wyłącznie numer lane'u** — kolejność wyników, `_manifest.json.snapshots[]`, JUnit i adresy `snapshots[i]` zostają kolejnością configu, a kolejność w obrębie lane'u też, więc scrub między snapshotami dalej jest w `scrubMs`, nie w `queuedMs`. Przy równych szacunkach plan degeneruje się dokładnie do round-robin. Domyślnie `parallel: 1`.
...
`value` w krokach `auth.login` = błąd walidacji; `auth.oauth` wyłącznie `*FromEnv`; `valueFromEnv` rozwiązywane w kliencie w czasie przebiegu (config parsuje się bez sekretu; brak zmiennej jest nazwany, `exit 2`). Składnia w sesji: `--env NAZWA` albo `@{NAZWA}` w `fill`/`form` (literały zaczynające się od `@` bez klamry są literałami; `@{FOO}` bez zmiennej = błąd nazwany, nie wpisanie „@{FOO}” do formularza). `describe` echuje tylko nazwę zmiennej. Keeper trzyma `secretValues` per zadanie/sesja i **redaguje** je (`redact()` z `src/redact.mjs`, jedna funkcja) w: stdout, `journal.jsonl`, logu, `report.md/json`, `snap.md`/`snap.full.yml`/`snap.json`, `extract`, `eval`, `text.txt`, `console.jsonl`, `net.jsonl`, eksporcie (tam `valueFromEnv`); pola `type=password` i `autocomplete=one-time-code` **nigdy** nie mają wartości w snapshocie — ani `= wartość` w `snap.md`, ani `: wartość` w `snap.full.yml`, ani `value` w `snap.json`. W `snap.full.yml` linii **nie** rozpoznaje wyrażenie po roli: nazwa dostępna bywa z dwukropkiem (`textbox "Hasło:"`), a przy `": "` renderer cytuje cały klucz apostrofami (`- 'textbox "Kod: SMS" [ref=e5] …': 123`) — dlatego wartość tnie się po dwukropku domykającym klucz, a **każda** linia, także ta bez wartości, przechodzi przez `redact` (sekret w nagłówku albo w `/url` też jest sekretem). Ref linii czyta się **za nazwą dostępną**, nie od początku klucza: nazwa jest tekstem strony i `aria-label="Hasło [ref=e1] konta"` podstawiał ref innego węzła (wartość zostawała), a w drugą stronę ucinał wartość polu, które wrażliwe nie jest — pomijanie nazwy obejmuje **obie** formy renderera: cytowaną i niecytowaną `/…/` (`createKey` zostawia bez cudzysłowów nazwę, która zaczyna się i kończy `/`, więc szukanie pierwszego `"` cofało maskę do stanu sprzed naprawy). Wartość, której sidecar nie może **potwierdzić**, jest cięta tak samo jak wrażliwa: linia o roli niosącej wartość **bez `[ref=]`** (pole pod `pointer-events: none` nie dostaje od playwrighta refa, więc nie ma wpisu w sidecarze, który mógłby je oznaczyć) oraz pole, dla którego obchód DOM nie znalazł **żadnego** elementu — nieznana wrażliwość nigdy nie znaczy „niewrażliwe". Wartość nie zawsze stoi po dwukropku klucza: gdy węzeł ma własność (`- /placeholder: …`), renderer przenosi ją do **liścia `- text:`** pod spodem — maska schodzi więc do poddrzewa pola, a kompakt bierze wartość stamtąd (inaczej `snap.md` pokazywał puste pole, w które agent przed chwilą wpisał). Wartość pola nie ma też prawa trafić w **slot nazwy** kompaktu ani w tekst kontenera (`dialog`, `alert`) — maska zna wyłącznie slot `= wartość`, więc pole bez etykiety renderuje się bez niej. Wrażliwość pola zna sidecar, więc jego przejście po DOM schodzi do **ramek potomnych i otwartych shadow rootów** — inaczej hasło w widżecie logowania w `<iframe>` albo w web komponencie zostaje jawne; ramkę dobiera się po **własnym refie węzła `iframe`** (`contentFrame()`), bo `page.frames()` zawiera także ramki, których drzewo aria nie pokazuje (`display:none`), a parowanie po pozycji dawało widżetowi obchód cudzego dokumentu. Gdy przejścia po DOM **nie da się wykonać** (nawigacja w trakcie, strona podmieniła prototyp DOM), wrażliwość jest NIEZNANA i snapshot tnie wartość **każdej** linii o roli niosącej wartość — fail-closed, bo alternatywą było ciche „nic nie jest wrażliwe". To samo dotyczy awarii **jednej ramki**: obchód, który padł albo odpowiedział za inny dokument, zostawiał pola tej ramki bez `sensitive`, podczas gdy sidecar jako całość wyglądał zdrowo — dlatego niedopasowanie liczy się **per pole**, a nie per snapshot (wartości ramek, które się udały, zostają). Redaktor obejmuje też oba pliki, które przebieg zostawia po snapshotach (`<stamp>/_manifest.json`, `--junit`), a formy sekretu to nie tylko surowa, JSON-owa i `encodeURIComponent`: także zapis formularza (`+` zamiast spacji) i base64. Jeden test przechodzi przez `fill/form/storage set/eval` i sprawdza wszystkie miejsca. Transport tylko lokalny, z tokenem, bez `env`. Nie ma ścieżek DELETE poza `storage … clear` w piaskownicy własnego kontekstu. `eval`/`evaluate` wykonuje JS **w kontekście strony**; `browser-inspector run --file s.mjs` (`export default async (page, context) => …` w keeperze) istnieje **wyłącznie w sesji pod `BROWSER_INSPECTOR_UNSAFE=1`** i jest jawnie nazwane RCE-równoważnym jak `run_code_unsafe` MCP — nigdy w configu batchu ani w CI.
...
`verify kind: list` = `{ do: 'verify', kind: 'list', selector|ref, items: […] }`: każdy `items[i]` jest widocznym tekstem potomka w podanej kolejności. `kind: text` i `kind: list` sprawdzają **widoczność przed tekstem**: `innerText` na elemencie, którego strona nie renderuje (`hidden`, `display:none`), zwraca `textContent`, więc bez tego asercja przechodziła na banerze błędu, którego nikt nie widzi. `verify` zatrzymuje flow jak każdy krok; `soft: true` (sesja: `--soft`) zapisuje wynik do `verifications[]`, nie zmienia `completed`, a `report.md` dostaje sekcję `## verify` z wierszami FAIL. `soft` obejmuje **także nierozwiązany cel**: martwy ref (usunięty modal — czyli dokładnie stan, który `verify hidden` ma sprawdzać) jest werdyktem `ok: false` z powodem w `detail`, nie wyjątkiem kroku; ref, którego nigdy nie było, tak samo — brak refa nigdy nie jest zielonym `hidden`.

`waitFor` to **oczekiwanie, nie asercja**, i dlatego rozstrzyga martwy ref inaczej niż `verify`: `waitFor <eN> --state hidden|detached` na refie, który **był w snapshotcie trzymanym przez agenta** i przestał się rozwiązywać, jest spełniony (element, którego nie ma, jest i ukryty, i odpięty) — inaczej krok padał dokładnie w chwili, w której jego warunek się spełniał, a wynik flow zależał od tego, czy aplikacja chowa przez `display:none`, czy przez usunięcie węzła. Ref, którego żaden snapshot nie pokazał (literówka, ref sprzed nawigacji, której agent nie odświeżył), dalej jest porażką, a `--state visible|attached` bez zmian.

`extract` czyta tekst węzła **niezależnie od renderowania** — `innerText` na węźle, którego strona nie renderuje, oddaje `textContent`, a zwinięty panel bywa prawidłowym celem. Odczyt zostaje, ale wartość przestaje udawać tekst z ekranu: krok oznacza ją `hidden: true` w `report.json`, a `## values` pisze `nazwa (hidden): …`. Gwarancję widoczności daje `verify kind: text|list`, nie `extract`.

### 3.3 Config batch — zgodność z app-factory

Obecny `read.config.browser-inspector.json` (6 snapshotów, `networkidle`, `wait ms`, `mat-option:has-text`, `evaluate` rzucający) parsuje się **bez zmian** i daje ten sam katalog `<outputDir>/<stamp>/<name>/report.json`; jest fixture’em. Honorowane pola: `outputDir`, `browser{channel, executablePath, headless, args}`, `auth{storageState, login|oauth, maxAgeMinutes, reuse}`, `snapshots[{name, type: page|flow, url, waitUntil, fullPage, viewport, steps, stepTimeoutMs, navTimeoutMs, captureElements, captureNetwork, auth: false, render}]`. Wiodący BOM (`Set-Content -Encoding UTF8` w Windows PowerShell 5.1) jest **zdejmowany** przed `JSON.parse` — tak samo tolerancyjnie jak `browser-inspector script`; `snapshots[i].storageState` **obok bloku `auth`** to błąd walidacji z podpowiedzią `"auth": false`, bo sesja bloku `auth` i tak by go przykryła i snapshot pojechałby na cudzym koncie. Ścieżka względna w `snapshots[i].storageState` liczy się od **katalogu configu** (tak jak `auth.storageState` i `outputDir`) — surowy string trafiał do `newContext`, które rozwiązuje go względem cwd procesu trzymającego przeglądarkę, czyli keepera, więc ten sam config jechał na innym koncie zależnie od katalogu startu. Nowe pola są opcjonalne:
  ...
  "snapshots": [
    {
      ...
      "waitUntil": "settled", "isolation": "reuse", "finalScreenshot": "auto", "captureSnapshot": false, "captureBodies": false,
...
`snap.full.yml` = surowe `ariaSnapshot({ mode: 'ai', boxes: true })`. `snap.json` = sidecar `[{ ref, role, name, selector?, box, sensitive?, valueUnknown?, inIframe? }]` z **box-joinu** (`valueUnknown` = pole niesie wartość, a obchód DOM nie znalazł dla niego elementu, więc jego wartość jest cięta jak wrażliwa; `inIframe` = węzeł siedzi pod węzłem `iframe`, więc jego selektor jest lokalny dla tamtego dokumentu — to stąd `export` wie o ramce, gdy sonda na żywym elemencie nie zdąży odpowiedzieć): jeden in-page `evaluate` zbiera prostokąty i trwałe selektory (`data-testid` → `#id` → `[name]`) elementów interaktywnych, a linie drzewa łączy się po zaokrąglonym `[box=…]` (136/136, 16 ms). `snap.md` = `compactSnapshot()`: zostają linie z `[ref=` o rolach interaktywnych (button, link, textbox, searchbox, combobox, option, checkbox, radio, switch, tab, menuitem, slider, spinbutton) i semantycznych (heading, alert, status, dialog, img z nazwą) oraz liście tekstu pod alert/status; wycięte `generic` bez nazwy, `[cursor=pointer]`, `[box]`; nagłówek renderuje się bez refa (`h2 "Nowości"` — orientuje, nie jest celem) **poza** nagłówkiem, który strona sama zrobiła celem (`[cursor=pointer]`, czyli akordeon `<h3 onclick>`): tam ref stoi na początku linii, bo innego adresu taki węzeł nie ma — `walkInteractive` go nie widzi, więc sidecar nie ma dla niego selektora; zwijanie rodzeństwa nie dotyczy serii, których element renderuje się do **jednej** linii (nazwa jest tam całą treścią, a sygnatura zwijania nazwy ignoruje, więc 12 różnych linków nawigacji stawało się jednym linkiem i trzema próbkami nazw, a `snap --diff` mówił `0 changed` o przemianowanej pozycji za zwinięciem); `- /url:` zwinięte do ` → /cart`; selektor z sidecara na końcu linii. Bez `--names` linia nie niesie kontekstu (19 tok.); `--names` dopisuje w nawiasie nazwę najbliższego przodka `article|li|[role=listitem]|section` (jego pierwszy heading albo `aria-label`, ≤ 60 znaków; 31 tok.). Bookstore: 952 linii YAML → ~80 linii:
...
- **Ramki**: pełny snapshot `ai` **zawiera iframe’y** z refami `f<seq>eN`, które rozwiązują się bez przełączania (fakt (d)); `browser-inspector frame 2` / `browser-inspector frame main` zmienia zakres tylko dla **selektorów CSS/tekstowych** (`frame.locator`) — czyli dla `click`, `fill`, `extract`, `waitFor`, `verify`, `shot --el` — nigdy nie robi snapshotu poddrzewa. `eval` bez `--el` idzie przez CDP, a ten zawsze wykonuje w ramce **głównej** (mapowanie wyniku §2.2 jest przypięte do tej ścieżki), więc pod ustawionym zakresem `eval` **odmawia** zamiast cicho odpowiedzieć o innym dokumencie: `eval --el <eN|selektor>` albo `frame main`. Zakres jest **per dokument**: nawigacja głównej ramki (`goto`, `reload`, `back`/`forward`, klik w link) odłącza ramki potomne, więc zakres wskazujący odłączoną ramkę jest porzucany przy pierwszym użyciu — bez tego każdy późniejszy selektor CSS kończył się `Frame was detached`, a w sesji aż do `frame main`. `frame 0` (ramka główna) nawigację przeżywa.
...
- **Eksport** (`src/session-log.mjs → exportFlow`): każda akcja na refie zapisuje selektor rozwiązany **w chwili akcji** (`locator('aria-ref=eN').evaluate(locatorFor)`, 14 ms) z preferencją `[data-testid=…]` → `#id` → `[name=…]` → `role=button[name="…"]`; wartości z `--env`/`@{NAZWA}` stają się `valueFromEnv`; eksport pisze **nowy plik** (istniejący wymaga `--force`); test: żadna wartość sekretu nie ląduje w eksporcie, dzienniku, raporcie ani stdout. Eksport obejmuje **wyłącznie bieżącą sesję**: `journal.jsonl` jest per NAZWA sesji i przeżywa `close`, recykling po TTL i wczorajszy dzień, więc każda linia niesie `sid` nadany przy `open`, a `exportFlow` bierze linie ostatniego `sid` (dziennik bez `sid` = jedna, starsza sesja). Bez tego eksport zaczynał od pierwszego `open` w pliku i cicho odtwarzał cudzy przebieg z cudzym URL-em startowym.
...
Przy porażce nagłówek to `# wizard-formularz — FAIL 9/16 · 1 812 ms · warm reused · final.png`, a sekcja `## steps` listuje dwa kroki przed nieudanym, nieudany i `skipped 11–16` (próbka: 139 tok.). Nagłówek dostaje `sw=blocked`, `motion=reduce`, `final: koszyk.png`, `tab new`, `fallback` tylko wtedy, gdy zachodzą. Wartości wieloliniowe w płotkach; > 5 000 znaków → `values/<name>.txt` z **całą** wartością (`report.json` trzyma głowę 5 000 znaków i `truncated: true`, a wiersz w `## values` podaje prawdziwą długość, nie „5 000+"). `elements.md` i licznik `el N` liczą także ramki potomne i otwarte shadow rooty, ale **listują** tylko ramkę główną — selektor z ramki nie rozwiązuje się ze strony, więc mapa mówi „niepełna” (`elements.truncated` liczone wobec tego, co raport LISTUJE, nie wobec capu 100), a drogą do wnętrza ramki jest snapshot (refy `f<seq>eN`). Każdy selektor mapy trafia w **jeden** element: `#id`, `[data-testid]` i `[name]` idą do raportu tylko wtedy, gdy są unikalne we wszystkich korzeniach (CSS Playwrighta przebija otwarte shadow rooty, a dwa web komponenty mają prawo do tego samego `id`). `text.txt` jest cięty do 20 000 znaków **w stronie**, więc flagę i prawdziwą długość niesie pomiar — `report.json.text.truncated` z `length`, a stopka `report.md` pisze `text.txt (20 000 of N)`. Sekcja `## verify` tylko przy miękkich porażkach. Stary celowany odczyt: 567 tokenów; tu cały plik ≈ 190.
