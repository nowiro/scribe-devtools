...
| `snapshot` | `snap` | `query` | config + sesja | `snap [--max 25] [--diff] [--around eN] [--grep text] [--names] [--all]   (filters: session only; a config writes the full snapshot)` | `name: name?`, `max: int?`, `diff: bool?`, `around: ref?`, `grep: string?`, `names: bool?`, `all: bool?` | `--max: int`, `--diff`, `--around: string`, `--grep: string`, `--names`, `--all` |
