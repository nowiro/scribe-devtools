...
<!-- INSTRUCTION:START -->
> Przeglądarka: `browser-inspector <config.json> [--stamp X]` wykonuje flow, wynik w `<outputDir>/<stamp>/<snapshot>/report.md` (`## errors`, `## values`; `## steps` tylko przy FAIL); nieudany krok = wynik, exit 0. Sesja: `browser-inspector open <url>`, `browser-inspector find <tekst>` / `browser-inspector snap` dają refy `eN`; `browser-inspector click|fill|form|press|select|wait|shot|eval|console|net …` drukują jedną linię (exit 1 = FAIL); `browser-inspector tools|call <tool> {json}` = narzędzia WebMCP strony; `browser-inspector export flow.json` zapisuje sesję jako config.
<!-- INSTRUCTION:END -->
...
| `oxfmt --check --disable-nested-config` | format kodu, JSON-a i YAML-a: 120 kolumn, LF, pojedyncze cudzysłowy, przecinki końcowe wszędzie (`.oxfmtrc.jsonc`; poza `.gitignore` wyklucza `ignorePatterns`). Zastąpił Biome z tymi samymi liczbami — na tym drzewie różnica wyszła w JEDNEJ linii kodu. `sortPackageJson` wyłączone. **Markdownu nie formatuje nikt, z wyboru**: oxfmt go umie, ale dopełnia tabele spacjami (ten plik +48 % bajtów) i wstawia puste linie w bloki `INSTRUCTION`. Szerokość i zawijanie prozy trzymasz ręcznie |
| `oxlint --disable-nested-config` | kategoria `correctness` jako błąd, `denyWarnings`, nieużyty `oxlint-disable` to też błąd (`.oxlintrc.json`; flaga, bo `copilot-builder/` ma własne konfiguracje oxc). Świadomy wyjątek: `// oxlint-disable-next-line <reguła> -- <powód>` nad linią, nigdy bez powodu |
