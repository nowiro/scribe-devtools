...
1. Zlecenie do wykonawcy (kontrakt z `orchestrator-sdd`): cel, pliki w zakresie, AC, brama do zaliczenia,
...
3. Po każdym zadaniu: `status → done` w planie, wiersz w run-logu (agent, tier, artefakt, wynik bramy).
   Jeden krok = jeden proponowany commit (`type(scope): subject`); commit wykonuje człowiek.
