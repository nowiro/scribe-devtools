---
description: 'Snapshot ALM przez skrypt alm:read: zrzut Jira/Confluence/GitLab/Sonar do .alm/ i czytanie go wybiórczo — zamiast serwera MCP'
agent: orchestrator
---
...
1. Config w korzeniu repozytorium: `read.config.<źródło>.json` (gitignorowany). Brak → skopiuj wzór:
   `tools/alm/examples/read.config.<źródło>.json`, ustaw zakres (JQL, `projectId`, `pageId`) i zostaw
   `outputDir` na `./.alm/<źródło>`. Nieznany klucz to głośny błąd — schemat jest ścisły.
...
4. Czytaj `.alm/<źródło>/<stempel>/<snapshot>/_manifest.json` (lista plików z rozmiarami w bajtach),
