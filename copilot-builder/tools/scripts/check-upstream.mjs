...
//   node tools/scripts/check-upstream.mjs --ack <id|all>  a human looked and is choosing to stay behind —
...
// `tools/scripts/upstream-state.json` — committed, so the clock survives across days and machines.
// Catching up to `latest` deletes the entry; the clock for the NEXT time we fall behind starts
// fresh. A WARN does not mean "you did something wrong" — `@types/node` is pinned to the Node major
// of `.nvmrc` on purpose (`pins.config.mjs`'s own `why` says so) and will warn every `staleDays` forever. It means
