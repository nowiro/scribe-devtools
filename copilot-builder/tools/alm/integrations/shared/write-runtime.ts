 ...
 * ('Utworzono za pomocą narzędzia alm v2 można poznać po…') was eaten whole.
 */
const TRAILING_PROVENANCE =
  /(?:^|\n)([_*]?)(?:Utworzono|Zaktualizowano|Dodano) za pomocą narzędzia alm v\d[\w.-]*\.\1$/u;
...
export function provenanceLine(action: ProvenanceAction): string {
  return `${PROVENANCE_VERB[action]} za pomocą narzędzia alm v${getRepoVersion()}.`;
}
