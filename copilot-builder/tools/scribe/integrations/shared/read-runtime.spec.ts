...
//   1. the raw material lands under a gitignored `.scribe/<source>/`, never in the tree;
...
describe('defaultOutputDir', () => {
  it('keeps every source inside the gitignored cache directory', () => {
    expect(defaultOutputDir('jira')).toBe('./.scribe/jira');
    expect(defaultOutputDir('gitlab')).toBe('./.scribe/gitlab');
  });
});
