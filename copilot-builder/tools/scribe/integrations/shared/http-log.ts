 ...
 *
 * Plik: `<EXTRACT_HTTP_LOG_DIR | ./.scribe/http-log>/<skrypt>-<stempel>-<pid>.jsonl`
 * — katalog `.scribe/` jest gitignorowany jak każde dane klienta. Wyłączenie:
...
export function createHttpLogger(scriptName: string, env: NodeJS.ProcessEnv = process.env): HttpLogger {
  ...
  const dir = resolve(env['EXTRACT_HTTP_LOG_DIR'] ?? './.scribe/http-log');
