# MCP vs scribe — porównanie kosztów i architektury

Dwa sposoby dania agentowi (Copilot, Claude, dowolny inny) dostępu do tych samych danych:
**serwer MCP** wystawiający narzędzia przez protokół, albo **scribe** — skrypty, które
...
flowchart LR
    ...
    subgraph SCR["scribe"]
      ...
      API2 --> CLI
      CLI -- "snapshot na dysk" --> FS[(".scribe/&lt;źródło&gt;/&lt;data&gt;/")]
...
2. **Koszt zmienny.** W MCP wynik każdego wywołania (pełny JSON zadania, strona, lista MR)
   przepływa przez kontekst agenta. W scribe wynik ląduje w plikach, a agent otwiera tylko
   te, których potrzebuje — grep po `.scribe/` jest tańszy niż transport wszystkiego przez
   okno.
...
| **scribe (8 źródeł + zapis)** | skrypty      |         0 |          **~50** |

```mermaid
xychart-beta
    title "Tokeny definicji wchodzace do kontekstu w KAZDEJ sesji (zmierzone)"
    x-axis ["jira", "gitlab", "confluence", "figma", "sonar", "scribe (8 zrodel)"]
...
- **scribe:** jedno zdanie w `.github/copilot-instructions.md` („snapshoty robi
  `npm run alm:read -- <źródło>`, zapis `npm run alm:create`/`update -- <źródło> plik.md`, wyniki w `.scribe/`") —
...
xychart-beta
    ...
    line "scribe" [50, 250, 500, 750, 1000]
```

Koszt zmienny działa w tę samą stronę, choć zależy od danych: pobranie sprintu 50 zadań
przez MCP to 50+ wywołań, których **pełne odpowiedzi przechodzą przez kontekst**; w scribe
...
- **żywa interaktywność** — „spójrz na stronę, potem zdecyduj, co kliknąć": to wymaga
  trwałego procesu; scribe świadomie pokrywa tylko część batchowalną (flow + mapa
  elementów),
- **dynamiczne odkrywanie narzędzi** w trakcie sesji i **potwierdzenia per wywołanie**
  w UI klienta,
- **OAuth po stronie klienta** — MCP-owy klient umie przeprowadzić logowanie; scribe
