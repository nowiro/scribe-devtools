...
//
// Regenerated by the pre-commit hook (`.githooks/pre-commit`, armed by `pnpm run prepare`) and
// guarded by `pnpm run verify` — a stale index fails the gate instead of silently drifting.
//
  ...
  if (process.argv.includes('--check')) {
    ...
    if (current === fresh) {
      ...
      process.stderr.write(
        `${INDEX_FILE} is STALE — run \`pnpm run code-index\` (the pre-commit hook does this automatically)\n`,
      );
      process.exitCode = 1;
    }
  } else {
