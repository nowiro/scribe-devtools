// The client's module graph (DESIGN.md §2.4, AC-13): `bi help` and a session command without a
...
/** Run bi under the trace hook and return the resolved module URLs. */
async function trace(args, overrides = {}) {
  const h = makeEnv(overrides);
  harnesses.push(h);
  const file = path.join(h.tmpdir, 'loads.txt');
  h.env.BI_TRACE_LOADS = file;
  const result = await bi(args, h, { nodeArgs: [`--import=${HOOK}`] });
...
describe('client module graph', () => {
  it('`bi help` loads the parser and the table, nothing that carries a browser', async () => {
    const { result, urls } = await trace(['help']);
    expect(result.code).toBe(0);
    expect(result.stdout).toContain('bi <config.json>');
    expect(urls.length).toBeGreaterThan(0);
    expect(urls.some((u) => /\/src\/steps\.schema\.mjs$/u.test(u))).toBe(true);
    // `bi help` needs the parser only — the client module with node:net stays unloaded.
  ...
  it('a session command without a keeper goes through the client and still never touches the engine', async () => {
    const { result, urls } = await trace(['click', 'e5'], { BI_DAEMON: '0' });
  ...
  it('a lint-config run (config loader, no keeper) stays engine-free too', async () => {
    ...
    const file = path.join(h.tmpdir, 'loads.txt');
    h.env.BI_TRACE_LOADS = file;
    const result = await bi(['lint-config', config], h, { nodeArgs: [`--import=${HOOK}`] });
    expect(result.code).toBe(0);
