...
describe('one keeper per identity', () => {
  it('auto-starts on `bi up`, listens before the engine, and two clients hit the same process', async () => {
    const h = fresh();
    const up = await bi(['up'], h);
    ...
    const [a, b] = await Promise.all([bi(['status'], h), bi(['status'], h)]);
    ...
    expect(a.lines[1]).toContain('bin/bi.mjs');
  ...
  it('cleans a stale pid file, lock and socket itself (the client never unlinks)', async () => {
    ...
    const pidPath = path.join(h.tmpdir, `bi-${identity.key}.json`);
    const lockPath = path.join(h.tmpdir, `bi-${identity.key}.lock`);
  ...
  it('BI_SOCKET is the pipe: status prints it and the pid file records it', async () => {
    const h = fresh();
    await bi(['up'], h);
    const status = await bi(['status'], h);
  ...
  it('`bi stop` ends the keeper and removes pid and lock', async () => {
    const h = fresh();
    await bi(['up'], h);
    const info = readPid(h);
...
describe('idle and sessions', () => {
  it('exits after BI_IDLE_MS only when the queue is empty', async () => {
    const h = fresh({ BI_IDLE_MS: '200' });
    await bi(['up'], h);
    const info = readPid(h);
  ...
  it('an open session blocks idle until `bi close`', async () => {
    const h = fresh({ BI_IDLE_MS: '200' });
    await bi(['up'], h);
    const info = readPid(h);
    const open = await bi(['open', 'http://localhost:4521/', '--session', 's'], h);
    expect(open.code).toBe(0);
    await sleep(600);
    expect(isAlive(info.pid)).toBe(true);
    const status = await bi(['status'], h);
    expect(status.lines.some((l) => l.startsWith('session s · cwd '))).toBe(true);
    const close = await bi(['close', '--session', 's'], h);
    expect(close.code).toBe(0);
    await until(() => !isAlive(info.pid), 2000);
    expect(isAlive(info.pid)).toBe(false);
  }, 20000);

  it('a session expires after BI_SESSION_TTL_MS and the keeper goes idle', async () => {
    const h = fresh({ BI_IDLE_MS: '200', BI_SESSION_TTL_MS: '400' });
    await bi(['up'], h);
    const info = readPid(h);
    await bi(['open', 'http://localhost:4521/', '--session', 't'], h);
    await sleep(250);
    expect(isAlive(info.pid)).toBe(true);
  ...
  it('a command that leaves no session open (per the engine) does not keep the keeper', async () => {
    ...
    await bi(['up'], h);
    const info = readPid(h);
    const click = await bi(['click', 'e404', '--session', 'x'], h);
...
describe('queues', () => {
  ...
  it('runs a batch through the keeper: lanes round-robin, first→warm, progress per snapshot', async () => {
    ...
    const first = await bi([config, '--stamp', '2026-09-01_10-00', '--parallel', '2'], h);
    ...
    const third = await bi([config, '--stamp', '2026-09-01_10-02', '--only', 'three-fail', '--fail-on-incomplete'], h);
    expect(third.code).toBe(1);
    const unknown = await bi([config, '--only', 'nope'], h);
    expect(unknown.code).toBe(2);
    expect(unknown.stdout).toContain('--only: no snapshot named nope');
  }, 30000);

  it('recycles the browser between jobs after BI_MAX_JOBS and the next call is `first` again', async () => {
    const h = fresh({ BI_MAX_JOBS: '2' });
    await bi(['up'], h);
    await bi(['wait', '5', '--session', 'r'], h);
    await bi(['wait', '5', '--session', 'r'], h);
...
describe('engine failures are results, not crashes', () => {
  it('a keeper whose engine cannot load answers exit 2 with the reason', async () => {
    const h = fresh({ BI_ENGINE_MODULE: path.join(makeEnv().tmpdir, 'no-such-engine.mjs'), BI_IDLE_MS: '300' });
    const up = await bi(['up'], h);
...
describe('secrets', () => {
  it('never land in the keeper log or the session journal (fill --env, @{NAME}, form)', async () => {
    ...
    await bi(['open', 'http://localhost:4521/', '--session', 's', '--out', out], h);
    const fill = await bi(['fill', 'e3', '@{SECRET_X}', '--session', 's'], h);
    expect(fill.code).toBe(0);
    expect(fill.stdout).not.toContain(secret);
    const env = await bi(['fill', 'e3', '--env', 'OTHER_Y', '--session', 's'], h);
    expect(env.code).toBe(0);
    const form = await bi(['form', 'e1=@{SECRET_X}', 'e2=plain', '--session', 's'], h);
    ...
    const batch = await bi([config, '--stamp', '2026-09-01_11-00'], h);
...
describe('a browser that cannot launch is an answer, never a hang', () => {
  it('batch through the keeper: exit 2 with FAIL E_BROWSER_MISSING and the attempts, the socket closes', async () => {
    const h = fresh({ BI_FAKE_LAUNCH_FAIL: '1', BI_IDLE_MS: '300' });
    const config = writeConfig(h, [{ name: 'one' }, { name: 'two' }]);
    const run = await bi([config, '--stamp', '2026-09-02_10-00'], h);
  ...
  it('the same batch with --no-daemon prints the same reason with exit 2', async () => {
    const h = fresh({ BI_FAKE_LAUNCH_FAIL: '1' });
    const config = writeConfig(h, [{ name: 'one' }]);
    const run = await bi([config, '--no-daemon', '--stamp', '2026-09-02_10-01'], h);
    expect(run.code).toBe(2);
    expect(run.stdout).toContain('FAIL E_BROWSER_MISSING: no usable browser.');
...
describe('stale files after a reboot (a recycled pid)', () => {
  ...
  it('`bi stop` without a keeper names the stale pid file', async () => {
    const h = fresh();
    const { computeIdentity } = await import('../src/client.mjs');
    const identity = computeIdentity({ env: h.env });
    const pidPath = path.join(h.tmpdir, `bi-${identity.key}.json`);
    ...
    const stop = await bi(['stop'], h);
    expect(stop.code).toBe(0);
    expect(stop.lines[0]).toContain('stale pid file');
    expect(stop.lines[0]).toContain(path.basename(pidPath));
    fs.unlinkSync(pidPath);
  }, 20000);
});

describe('recycling waits for the sessions', () => {
  it('a recycle due under an open session is deferred and runs after `bi close`', async () => {
    const h = fresh({ BI_MAX_JOBS: '2' });
    await bi(['open', 'http://localhost:4521/', '--session', 's'], h);
    await bi(['wait', '5', '--session', 's'], h);
    await sleep(150);
...
describe('the scrub after the answer', () => {
  it('runs in the lane queue after the done line; the next client on the lane pays it as queuedMs', async () => {
    ...
    const h = fresh({ BI_FAKE_SCRUB_MS: '600' });
    const config = writeConfig(h, [{ name: 'a' }]);
    const first = await bi([config, '--stamp', '2026-09-02_11-00'], h);
    expect(first.code).toBe(0);
    expect(first.ms).toBeLessThan(2000);
    const second = await bi([config, '--stamp', '2026-09-02_11-01'], h);
    expect(second.code).toBe(0);
    ...
    const status = await bi(['status'], h);
    expect(status.lines[0]).toMatch(/queued [01]/u);
    await until(() => fakeLog(h).filter((e) => e.event === 'scrub').length === 2, 3000);
    expect((await bi(['status'], h)).lines[0]).toContain('queued 0');
  }, 20000);
});
...
describe('auth in a config is executed, not only validated', () => {
  ...
  it('a login that cannot run is FAIL E_AUTH with exit 2 — no snapshot runs anonymously', async () => {
    ...
    const run = await bi([config, '--stamp', '2026-09-02_12-01'], h);
    expect(run.code).toBe(2);
...
describe('secrets are per session', () => {
  it('a value filled with @{NAME} is still *** in a later `get` that carries no secrets', async () => {
    ...
    await bi(['open', 'http://localhost:4521/', '--session', 's'], h);
    await bi(['fill', 'e1', '@{SECRET_X}', '--session', 's'], h);
    const get = await bi(['get', 'e1', '--value', '--session', 's'], h);
...
describe('doctor', () => {
  it('spawns a probe keeper from a shell and reports whether it survived', async () => {
    const h = fresh();
    const result = await bi(['doctor'], h);
    expect(result.code).toBe(0);
    expect(result.lines[0]).toMatch(
      /^ok keeper survives shell: yes · spawn→listen [\d ]+ ms · first job [\d ]+ ms · warm [\d ]+ ms · hash [0-9a-f]{8} · .*bin\/bi\.mjs$/u,
    );
