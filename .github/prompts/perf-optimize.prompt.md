...
- Repository index — is there a generated map of the codebase at all, and is the agent TOLD to read it first? If one exists: is it generated or hand-written, is it regenerated by a hook, does `verify` fail on a stale one, does it carry signatures and not just names, and is its size stated anywhere the agent will see it, in a unit that is named? Time a realistic "find where X is handled" both ways — index read versus repository-wide grep — and count the tokens each pulls into context, because that difference is the whole case for or against.
...
Only when Nx is absent, or when the user asks for the comparison explicitly, is the four-way question live:
...
| oxfmt       | Prettier's coverage and output (Angular control flow, `.md`, `.yaml` included)              | one binary, plus a new editor extension | `.oxfmtrc.jsonc`; reads `.gitignore` and `.prettierignore`    | whole tree in tens of milliseconds                    |
...
- **oxfmt closes Biome's language gap, so check what it would ADD, not only what it keeps.** `npx oxfmt --migrate prettier` (or `biome`) writes `.oxfmtrc.json` from the old config; verify the ignore list by hand — Biome's `!!` "do not crawl" entries did not survive it here. Then `--list-different` on a scratch copy and read the diff by file type: by default oxfmt sorts `package.json` keys (`sortPackageJson: false` to keep manifests), adds trailing commas to `.jsonc`, and formats Markdown — which pads every table to its widest cell (+48 % bytes on this repo's AGENTS.md) and inserts blank lines inside HTML-comment markers. In a repo whose prose is read by agents, that is a token cost to put in the question, and `**/*.md` in `ignorePatterns` is the answer that keeps it out.
