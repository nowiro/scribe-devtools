...
Środowisko: 2026-09-02T13:00:45.840Z · 11th Gen Intel(R) Core(TM) i7-11850H @ 2.50GHz (16 rdzeni, 32 GB) · win32 10.0.26200 · Node 26.5.0 · browser-inspector 0.1.0 · playwright-core 1.62.1 · Chrome/152 · @playwright/mcp 0.0.80 (24 narzędzi w `tools/list`).
...
| --- | ---: | ---: | --- | ---: | ---: | ---: |
| browser-inspector-warm | **304 ms** | 326 ms | 10 · `warm` | **10,1×** | **11,6×** | **3,7×** |
| browser-inspector-warm-tight | **296 ms** | 319 ms | 10 · `warm` | **10,4×** | **11,9×** | **3,8×** |
| browser-inspector-first | **1372 ms** | 1375 ms | 3 · `first` | **2,6×** (vs 1. przebieg) | **3,1×** (vs 1. przebieg) | **1,3×** (vs 1. przebieg) |
| browser-inspector-cold | **1521 ms** | 1523 ms | 3 · `no-daemon` | **2,4×** (vs 1. przebieg) | **2,8×** (vs 1. przebieg) | **1,1×** (vs 1. przebieg) |
| browser-inspector-warm-fresh | **742 ms** | 746 ms | 10 · `warm` | **4,1×** | **4,7×** | **1,5×** |
| browser-inspector-first, first-ever (pierwsze w tym przebiegu benchu, n=1, poza ilorazami) | 1364 ms | — | 1 · `first` | — | — | — |
| MCP naive (agent poznaje ekran) | 3066 ms (warm) | 3093 ms | 10 · 1. przebieg 3634 ms | — | — | — |
| MCP lean (agent zna selektory) | 3522 ms (warm) | 3547 ms | 10 · 1. przebieg 4231 ms | — | — | — |
| MCP lean `--timeout-settle 100` | 1115 ms (warm) | 1136 ms | 10 · 1. przebieg 1739 ms | — | — | — |

**Wniosek z tabeli:** ścieżka ciepła `browser-inspector-warm` (mediana 304 ms, p90 326 ms) jest **10,1× vs domyślne** ustawienia MCP (naive warm 3066 ms), 11,6× vs MCP lean i **~3,7× vs zestrojony settle 100** (1115 ms) — dwie trzecie różnicy to domyślna polityka `--timeout-settle 500` serwera po każdej akcji, nie architektura. 5× jest własnością **każdego wywołania po pierwszym**; `browser-inspector-first` (1372 ms) i `browser-inspector-cold` (1521 ms) to fizyka startu Chrome i są raportowane osobno, poza progiem 5×.
...
xychart-beta
    ...
    y-axis "ms" 0 --> 4000
    bar [304, 296, 742, 3066, 3522, 1115]
```
...
| --- | ---: | ---: | ---: | ---: | --- |
| browser-inspector-interactive-naive | 17 | 2328 ms | 107 / 152 ms | 799 | ok |
| browser-inspector-interactive-lean | 17 | 2241 ms | 106 / 142 ms | 612 | ok |

<details><summary>browser-inspector-interactive-naive — komendy i stdout</summary>

```
$ browser-inspector open http://localhost:4300/   # 556 ms, exit 0
ok open "Zgłoszenie serwisowe" · el 12 · err 0 · .scribe-devtools/browser-inspector/session/bench-naive/snap.md
$ browser-inspector snap   # 116 ms, exit 0
...
$ browser-inspector click e28   # 152 ms, exit 0
ok click e28 · dom Δ
$ browser-inspector snap --diff   # 103 ms, exit 0
0 changed · .scribe-devtools/browser-inspector/session/bench-naive/snap.md
$ browser-inspector get [data-testid=error-email]   # 96 ms, exit 0
Podaj poprawny adres e-mail.
$ browser-inspector shot walidacja   # 107 ms, exit 0
ok shot .scribe-devtools/browser-inspector/session/bench-naive/shots/001-walidacja.png 1280x720
$ browser-inspector form "e10=Jan Kowalski" e12=jan.kowalski@example.com "e24=Formularz nie zapisuje zgloszenia po kliknieciu Wyslij."   # 126 ms, exit 0
ok form 3 fields · dom Δ
$ browser-inspector select e14 zmiana   # 107 ms, exit 0
ok select e14 = zmiana
$ browser-inspector click e22   # 121 ms, exit 0
ok click e22 · dom Δ
$ browser-inspector click e26   # 134 ms, exit 0
ok click e26 · dom Δ
$ browser-inspector click e28   # 125 ms, exit 0
ok click e28 · dom Δ
$ browser-inspector wait --sel [data-testid=confirmation]   # 100 ms, exit 0
ok wait [data-testid=confirmation]
$ browser-inspector get [data-testid=ticket-id]   # 100 ms, exit 0
ALM-1001
$ browser-inspector get [data-testid=ticket-category]   # 90 ms, exit 0
zmiana
$ browser-inspector get [data-testid=ticket-priority]   # 94 ms, exit 0
krytyczny
$ browser-inspector console --errors   # 96 ms, exit 0
2 new:
...
$ browser-inspector shot potwierdzenie   # 102 ms, exit 0
...
$ browser-inspector form "#name=Jan Kowalski" #email=jan.kowalski@example.com "#description=Formularz nie zapisuje zgloszenia po kliknieciu Wyslij."   # 117 ms, exit 0
ok form 3 fields · dom Δ
$ browser-inspector select #category zmiana   # 96 ms, exit 0
ok select #category = zmiana
$ browser-inspector click [data-testid=priority-krytyczny]   # 121 ms, exit 0
ok click [data-testid=priority-krytyczny] · dom Δ
$ browser-inspector click #consent   # 122 ms, exit 0
ok click #consent · dom Δ
$ browser-inspector click e28   # 131 ms, exit 0
ok click e28 · dom Δ
...
$ browser-inspector get [data-testid=ticket-category]   # 90 ms, exit 0
zmiana
$ browser-inspector get [data-testid=ticket-priority]   # 98 ms, exit 0
krytyczny
$ browser-inspector console --errors   # 86 ms, exit 0
2 new:
error Failed to load resource: the server responded with a status of 404 (Not Found) (http://localhost:4300/api/zgloszenia:0)
error [zgloszenia] zapis nie powiodl sie: HTTP 404 (http://localhost:4300/:125)
$ browser-inspector shot potwierdzenie   # 110 ms, exit 0
...
| --- | --- | ---: | ---: | --- |
| cmd | **yes** | 595 ms | 119 ms |  |
| bash | **yes** | 581 ms | 104 ms |  |
| pwsh | **yes** | 790 ms | 96 ms |  |
...
| --- | ---: | --- | ---: | --- |
| bez zmian | 1 | 12 595 · 10 140 | 6/6 | warm, warm |
| bez zmian | 3 | 5872 · 4307 | 6/6 | warm, warm |
| po migracji `settled` | 1 | 9520 · 8970 | 6/6 | warm, warm |
| po migracji `settled` | 3 | 3805 · 3751 | 6/6 | warm, warm |
