...
Generowany przez `npm run bench` (2026-09-02T13:42:29.870Z). Kolumna „projekt” to tabela §6 DESIGN.md pogrupowana tak,
...
| --- | ---: | ---: | ---: | ---: |
| 🔴 klient: start (72) + pipe (5) + dispatch (2) + wydruk i wyjście (6); `browser-inspector-first`: + import playwright-core (270), bo klient czeka na keepera w tym czasie; `browser-inspector-cold`: + context/browser.close (200) | 🔴 85 / **107** (+26 %) | 🔴 85 / **107** (+26 %) | 355 / **443** (+25 %) | 277 / **290** (+5 %) |
| spawn keepera → nasłuch (`_manifest.timing.keeperStartMs`) | — / **0** (—) | — / **0** (—) | 45 / **43** (−4 %) | — / **0** (—) |
...
| 🔴 launch Chrome (300) + kontekst i strona (115); `browser-inspector-cold`: + import playwright-core (270) w procesie | 0 / **3** (+3 ms) | 0 / **3** (+3 ms) | 🔴 415 / **200** (−52 %) | 🔴 685 / **489** (−29 %) |
| 🔴 goto `load` (ta sama karta 55 / świeży kontekst 250) | 🔴 55 / **26** (−53 %) | 🔴 55 / **24** (−56 %) | 🔴 250 / **358** (+43 %) | 🔴 250 / **360** (+44 %) |
| 🔴 18 kroków: waitFor 10 + 5×click 220 + 4×fill 40 + select 12 + waitFor 40 + 6×extract/evaluate 12 + 2×screenshot 50 | 🔴 384 / **174** (−55 %) | 🔴 384 / **176** (−54 %) | 384 / **290** (−24 %) | 384 / **299** (−22 %) |
| 🔴 dowód końcowy (el-count + title/text/elements) | 🔴 12 / **4** (−67 %) | 🔴 12 / **4** (−67 %) | 12 / **9** (−25 %) | 🔴 12 / **8** (−33 %) |
| 🔴 writeMs: ogon zapisu zrzutow (shotsMs) + zalegle odczyty cial przez recorder (settleMs) | 🔴 10 / **0** (−100 %) | 🔴 10 / **0** (−100 %) | 🔴 10 / **0** (−100 %) | 🔴 10 / **0** (−100 %) |
| 🔴 **razem** (klient spawn → exit) | 🔴 546 / **315** (−42 %) | 🔴 561 / **317** (−43 %) | 1469 / **1603** (+9 %) | 1618 / **1694** (+5 %) |
...
MCP naive warm: **2886 ms** · MCP naive 1. przebieg: 3761 ms · MCP lean `--timeout-settle 100` warm: 958 ms.

| wariant browser-inspector | mediana | p90 | podstawa | iloraz (pomiar) | iloraz (projekt §6) | vs lean settle 100 |
| --- | ---: | ---: | --- | ---: | ---: | ---: |
| browser-inspector-warm | 315 ms | 429 ms | MCP naive warm 2886 ms | **9,2×** | 5,3× | 3,0× |
| browser-inspector-warm-tight | 317 ms | 327 ms | MCP naive warm 2886 ms | **9,1×** | 5,2× | 3,0× |
| 🔴 browser-inspector-first | 1603 ms | 1605 ms | MCP naive, 1. przebieg 3761 ms | **2,3×** | 2,4× | 0,6× |
| 🔴 browser-inspector-cold | 1694 ms | 1696 ms | MCP naive, 1. przebieg 3761 ms | **2,2×** | 2,2× | 0,6× |
| 🔴 browser-inspector-warm-fresh | 852 ms | 880 ms | MCP naive warm 2886 ms | **3,4×** | 3,1× | 1,1× |
...
| --- | ---: | ---: | --- |
| `browser-inspector-warm-fresh` (`--fresh`: świeży kontekst z puli spare, goto 190–441) | 932 ms | 852 ms (goto 428 ms) | −9 % |
| app-factory, 6 snapshotów, `--parallel 1`, config bez zmian (completed 6/6) | ≈ 8000–10 000 ms | 10 636 ms (n=2) | poza zakresem, w tolerancji |
| 🔴 scrub między snapshotami app-factory (`snapshots[].scrubMs`, 4 originy) | 12–35 ms | 8 ms (n=10) | −33 % poniżej dołu |
| 🔴 app-factory, 6 snapshotów, `--parallel 3`, config bez zmian (completed 6/6) | ≈ 8000–10 000 ms | 4836 ms (n=2) | −40 % poniżej dołu |
| 🔴 app-factory, 6 snapshotów, `--parallel 1`, config po migracji `networkidle` → `settled` (completed 6/6) | ≈ 2500 ms | 8421 ms (n=2) | +237 % |
| 🔴 app-factory, 6 snapshotów, `--parallel 3`, config po migracji `networkidle` → `settled` (completed 6/6) | ≈ 1300–1600 ms | 3566 ms (n=2) | +123 % ponad górę |
