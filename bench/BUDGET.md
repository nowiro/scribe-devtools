...
Generowany przez `npm run bench` (2026-09-02T05:45:38.102Z). Kolumna „projekt” to tabela §6 DESIGN.md pogrupowana tak,
...
| --- | ---: | ---: | ---: | ---: |
| 🔴 klient: start (72) + pipe (5) + dispatch (2) + wydruk i wyjście (6); `browser-inspector-first`: + import playwright-core (270), bo klient czeka na keepera w tym czasie; `browser-inspector-cold`: + context/browser.close (200) | 🔴 85 / **109** (+28 %) | 85 / **103** (+21 %) | 355 / **404** (+14 %) | 277 / **268** (−3 %) |
...
| 🔴 launch Chrome (300) + kontekst i strona (115); `browser-inspector-cold`: + import playwright-core (270) w procesie | 0 / **3** (+3 ms) | 0 / **3** (+3 ms) | 🔴 415 / **185** (−55 %) | 🔴 685 / **438** (−36 %) |
| 🔴 goto `load` (ta sama karta 55 / świeży kontekst 250) | 🔴 55 / **22** (−60 %) | 🔴 55 / **20** (−64 %) | 🔴 250 / **344** (+38 %) | 🔴 250 / **346** (+38 %) |
| 🔴 18 kroków: waitFor 10 + 5×click 220 + 4×fill 40 + select 12 + waitFor 40 + 6×extract/evaluate 12 + 2×screenshot 50 | 🔴 384 / **159** (−59 %) | 🔴 384 / **157** (−59 %) | 🔴 384 / **256** (−33 %) | 🔴 384 / **255** (−34 %) |
| 🔴 dowód końcowy (el-count + title/text/elements) | 🔴 12 / **3** (−75 %) | 🔴 12 / **3** (−75 %) | 🔴 12 / **6** (−50 %) | 🔴 12 / **7** (−42 %) |
| 🔴 oczekiwanie na zapisy + report.json/md, elements.md, text.txt, manifesty | 🔴 10 / **43** (+330 %) | 🔴 10 / **47** (+370 %) | 🔴 10 / **15** (+50 %) | 🔴 10 / **15** (+50 %) |
| 🔴 **razem** (klient spawn → exit) | 🔴 546 / **348** (−36 %) | 🔴 561 / **333** (−41 %) | 1469 / **1478** (+1 %) | 1618 / **1548** (−4 %) |
...
MCP naive warm: **2886 ms** · MCP naive 1. przebieg: 3661 ms · MCP lean `--timeout-settle 100` warm: 1025 ms.

| wariant browser-inspector | mediana | p90 | podstawa | iloraz (pomiar) | iloraz (projekt §6) | vs lean settle 100 |
| --- | ---: | ---: | --- | ---: | ---: | ---: |
| browser-inspector-warm | 348 ms | 365 ms | MCP naive warm 2886 ms | **8,3×** | 5,3× | 2,9× |
| browser-inspector-warm-tight | 333 ms | 337 ms | MCP naive warm 2886 ms | **8,7×** | 5,2× | 3,1× |
| 🔴 browser-inspector-first | 1478 ms | 1533 ms | MCP naive, 1. przebieg 3661 ms | **2,5×** | 2,4× | 0,7× |
| 🔴 browser-inspector-cold | 1548 ms | 1557 ms | MCP naive, 1. przebieg 3661 ms | **2,4×** | 2,2× | 0,7× |
| 🔴 browser-inspector-warm-fresh | 796 ms | 828 ms | MCP naive warm 2886 ms | **3,6×** | 3,1× | 1,3× |
...
| --- | ---: | ---: | --- |
| `browser-inspector-warm-fresh` (`--fresh`: świeży kontekst z puli spare, goto 190–441) | 932 ms | 796 ms (goto 397 ms) | −15 % |
| app-factory, 6 snapshotów, `--parallel 1`, config bez zmian (completed 6/6) | ≈ 8000–10 000 ms | 10 854 ms (n=2) | poza zakresem, w tolerancji |
| 🔴 scrub między snapshotami app-factory (`snapshots[].scrubMs`, 4 originy) | 12–35 ms | 8 ms (n=10) | −33 % poniżej dołu |
| 🔴 app-factory, 6 snapshotów, `--parallel 3`, config bez zmian (completed 6/6) | ≈ 8000–10 000 ms | 5103 ms (n=2) | −36 % poniżej dołu |
| 🔴 app-factory, 6 snapshotów, `--parallel 1`, config po migracji `networkidle` → `settled` (completed 6/6) | ≈ 2500 ms | 8824 ms (n=2) | +253 % |
| 🔴 app-factory, 6 snapshotów, `--parallel 3`, config po migracji `networkidle` → `settled` (completed 6/6) | ≈ 1300–1600 ms | 4226 ms (n=2) | +164 % ponad górę |
